...
export const DnaStrands: FC<DnaStrandsProps> = (props) => {
  ...
  const handleScroll = () => {
    ...
    if (strandsRef.current?.children) {
      Array.from(strandsRef.current.children).forEach((strand: any) => {
        ...
        strand.children[0].style.transform = `
          translateY(${
            ((100 - scrollPercentage.current) * strand.dataset.offset) / 100
          }%)
        `;
      });
    }
    ...
    scrollPercentage.current > 98
      ? setDnaInteraction(true)
      : setDnaInteraction(false);
  };
   ...
   */
  strandsRef?.current?.querySelectorAll("svg rect").forEach((el: any) => {
  ...
  const handleOpacity = (e: any) => {
    ...
    if ($el && $el.nodeName && $el.nodeName === "rect") {
      ...
      strandsRef?.current?.querySelectorAll("svg rect").forEach((rect: any) => {
      ...
      strandsRef?.current?.querySelectorAll("svg rect").forEach((rect: any) => {
        ...
        let $descriptors = document.querySelectorAll(".strand-descriptor");
        $descriptors?.forEach(
          (descriptor: any) => (descriptor.style.opacity = "0")
        );
      });
    }
  };
...
const Strand: FC<StrandProps> = (props) => {
  ...
  return (
    <div
      className={`relative flex flex-col items-start el-strand`}
      data-offset={offset}
    >
      <div
